...
- **Buffer mode:** &nbsp;Press `ctrl+a` to enable buffer mode, which will redirect all received messages to a temporary buffer. You can then type your long message in peace without worrying about incoming messages overwriting them. Pressing `ENTER` will retreive all the held messages and disable buffer mode.
